-- --------------------------------------------------------------------------------
-- Automatically generated by Reggae EDA tool
-- Copyright Jean-Christophe Le Lann
-- --------------------------------------------------------------------------------
library ieee;
...
begin
  ...
  --------------------------------------------------------
  tick_gen_i: entity uart_lib.uart_tick_gen
  port map(
    reset_n => reset_n,
    clk100  => clk100,
    tick    => tick);
  ...
  -- receiver
  -------------------------------------------------------
  rcvr_i : entity uart_lib.uart_receiver
    port map (
      reset_n    => reset_n,
      clk100     => clk100,
      tick       => tick,
  ...
  rcv_to_ip_fifo : entity uart_lib.fifo
    generic map(
      NB_BITS_DATA => 8,
      FIFO_SIZE    => 8)
    port map(
  ...
  -- sender
  -------------------------------------------------------
  sender_i : entity uart_lib.uart_sender
  port map(
    reset_n      => reset_n,
    clk100       => clk100,
    tick         => tick,
  ...
  ip_to_sndr_fifo : entity uart_lib.fifo
    generic map(
      NB_BITS_DATA => 8,
      FIFO_SIZE => 8)
    port map(
